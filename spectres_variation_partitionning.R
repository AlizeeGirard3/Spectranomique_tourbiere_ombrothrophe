...
refl_norm_large <- rt_sg_normalized_large %>% 
  dplyr::filter(propriete == "reflectance") %>% 
  dplyr::select(-"propriete", -"site", -"normalization_magnitude")
# names(refl_norm_large)[4:2003] <- paste("X", names(refl_norm_large)[4:2003], sep="")
# issu du script "spectre_normalisation.R" dans document scripts-NETTOYAGE

load("~/Documents/Maîtrise/DonnéesAnalyses/traits_fonctionnels/ft_all_vX1.RData")
ft_all <- ft_all_vX1 %>% select(site_class, everything(),-recalcitrant, -Phenols_mg_g, -Tannins_mg_g)
