...
# GROUPE TF "pigments" (3) : "VIS" 400:760 nm / chlA, chlB, caroténoides, A_B
...
refl <- rt_sg_large %>%  # autres colonnes intéressantes, voir colnames(rt_sg_large)
  select(c("400":"760")) %>%  
...
# 11 juillet, analyse TF "pigments"
# 8 oct, analyse comparative avec spectres normalisés
...
pigments_df <- refl %>% 
  dplyr::select(sample_id, chlA_mg_g, chlB_mg_g, carotenoides_mg_g, chlA_chlB, chlA_mg_m2, chlB_mg_m2, "X400":"X760") # ratio A_B au besoin, sélectionner ici

# créer objets utilisés dans boucle 
inBands <- names(pigments_df)[8:368]
inVars <- names(pigments_df)[2:7]
...
for (i in 1:length(inVars)){
  ...
  nCut <- floor(0.7 * nrow(pigments_df)) # 80% data for calibration
  nComps <- 15
  nsims <- 100
  ...
  for (nsim in seq(nsims)){
    ...
    flush.console() # ?
    pigments_df$RAND <- order(runif(nrow(pigments_df))) # runif(n, min = , max = ) is used to generate n uniform (autant de fréquence de chaque tranche) random numbers lie in the interval (min, max).
    subData <- pigments_df[pigments_df["RAND"] < nCut, ] # choisi ligne < à ordre random, subset pour tester
  ...
  jpeg(paste("resultats/", "400-760_", inVar, "/", inVar, "_abs_PRESS.jpg", sep = ""),
  ...
  jpeg(paste("resultats/", "400-760_", inVar, "/", inVar,"_abs_RMSEP.jpg",sep = ""),
       width = 6, height = 5, units = "in", res = 200)
  par(bty = "l")
...
VIPjh = function(object, j, h) {    # où VIP : Variable Importance in Projection
...
(inVar <- names(pigments_df[2])) 

# subsetter saveData (issu de la partie 1)
pigments_df_ITER <- saveData[complete.cases(saveData[,inVar]),]
...
for (i in 1:length(nCompss)){
  nsims <- 100
  nComps <- nCompss[i]
  nCut <- floor(0.7*nrow(pigments_df_ITER))
  ...
  for (nsim in seq(nsims)){
    print(nsim)
    flush.console()
    pigments_df_ITER$RAND <- order(runif(nrow(pigments_df_ITER))) # same de partie 1 : juste créer chiffre random sur elsquels baser les subsets test et training
    subData <- pigments_df_ITER[pigments_df_ITER["RAND"] < nCut,]
    tstData <- pigments_df_ITER[pigments_df_ITER["RAND"] >= nCut,] # ">=" ne pas séparer
  ...
  write.csv(statMat, paste("resultats/", "400-760_", inVar, "/", inVar, "_", nComps,"comps_abs_stats.csv", sep=""), row.names=FALSE)
  ...
  specMat <- pigments_df_ITER[, inBands] # ne sort que les colonnes inBands d'pigments_df_ITER
  ...
  preds <- pigments_df_ITER[, !(names(pigments_df_ITER) %in% inBands | names(pigments_df_ITER) == "RAND")] # 2 critères de sélection des colonnes d'pigments_df_ITER à mettre dans
  # preds : [ {base} "x | y" = "OR" ] / DONC pas les noms d'pigments_df_ITER 
  # qui sont dans inBands OU METTRE les colonnes names(pigments_df_ITER) == "RAND")...
  ...
  write.csv(preds, paste("resultats/", "400-760_", inVar, "/", inVar, "_", nComps,"comps_abs_preds.csv", sep=""), row.names=FALSE)
  ...
  jpeg(paste("resultats/", "400-760_", inVar,  "/", inVar, "_", nComps, "comps_abs_predplot.jpg", sep = ""),
  ...
  # arrows(predMean, pigments_df_ITER[, inVar], predMean + predStdv, pigments_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  # arrows(predMean, pigments_df_ITER[, inVar], predMean - predStdv, pigments_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  ...
  # jpeg(paste("resultats/", "400-760_", inVar, "/", inVar, "_", nComps,"comps_abs_varimp.jpg", sep=""),
...
for (i in 1:length(nCompss)){
  ...
  # enregistrer
  # write.table(coeffis, paste("resultats//", inVar, "/", inVar, "_", nComps, "comps_abs_coeffMEAN.csv", sep = ""), 
  #             sep = ",", col.names = T, row.names = F)
