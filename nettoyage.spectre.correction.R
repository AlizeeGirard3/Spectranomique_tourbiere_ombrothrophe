...
# load("~/Documents/Maîtrise/DonnéesAnalyses/spectres/rt_sg.RData") # rt_sg
...
# obtenu via manips sur ce script sur # rt_sg (son équivalent, mais long)
...
# reflectance
refl <- as.matrix(spectra_large_refl) 
sample_id_provisoire <- rownames(refl) # spec_xxxxxxx en "rowname", retransformer en vecteur
sample_id <- gsub( "spec_", "", as.character(sample_id_provisoire)) # enlever "spec_"
refl.1 <- data.table(refl)
refl.2 <- cbind(sample_id, refl.1) # ajouter au df

# transmittance
trans <- as.matrix(spectra_large_trans) 
sample_id_provisoire <- rownames(trans) # spec_xxxxxxx en "rowname", retransformer en vecteur
sample_id <- gsub( "spec_", "", as.character(sample_id_provisoire)) # enlever "spec_"
trans.1 <- data.table(trans)
trans.2 <- cbind(sample_id, trans.1) # ajouter au df


#-=-=-=-=-=-=-=-=-=-=-=-=-
# Un fichier RT (à partir de l'objet NON-LISSÉ->12-02-2020) ----
#-=-=-=-=-=-=-=-=-=-=-=-=-

# colonne réfl dans le df réfl
propriete <- rep("reflectance", times = nrow(refl.2))  
refl.3 <-  refl.2 %>% 
  cbind(propriete) %>% 
  dplyr::select("sample_id", "propriete", c("400":"2400"))

# colonne trans dans le df trans
propriete <- rep("transmittance", times = nrow(trans.2))  
trans.3 <-  trans.2 %>% 
  cbind(propriete) %>% 
  dplyr::select("sample_id", "propriete", c("400":"2400"))

# joindre reflectance et transmittance
rt_provisoire1 <- refl.3 %>% 
  full_join(trans.3)
...
# joindre site et scientific_name à rt
rt_provisoire2 <- left_join(rt_provisoire1, site, by = 'sample_id') # left_join enlèvera les sample_id qui sont dans gauche et pas le droite
rt <- left_join(rt_provisoire2, scientific_name, by = 'sample_id') %>% 
...
rt_long <- gather(rt, key = "wavelength", value = "valeur", c("400":"2400"), -sample_id, -scientific_name, -site)
rt_long$wavelength <- as.numeric(rt_long$wavelength)
# save(rt_long, file = "~/Documents/Maîtrise/DonnéesAnalyses/spectres/rt_long.RData") # rt_long / refait le 29 mai avec matched_sensors() / refait 21 juin données officielles / refait 12 février 2020, enlevé le lissage !
...
#ggsave("rt.pdf", plot = cinq_echantillons_rt, height = 10, width = 10)
# aucune différence à présent puisque pas de lissage
...
sg_data_NIR_detect_lis <- rt_long %>%
  dplyr::filter(wavelength >= 900,
                wavelength <= 1090 ) %>%
  group_by(sample_id, site, scientific_name, propriete) %>% 
...
rt_sg <- bind_rows(sg_data_VIS_NIR1_lis, sg_data_NIR_detect_lis,
...
#-=-=-=-=-=-=-=-=-=-=-=-=-
# séparer R et T
rt_sg$wavelength <- as.numeric(as.character(rt_sg$wavelength))
...
rt_long$wavelength <-  as.numeric(as.character(rt_long$wavelength))

x11670314_r_ORIGINAL <- rt_long %>%
  dplyr::filter(propriete == 'reflectance',
                sample_id == 11670314)
# x11670314_t_ORIGINAL <- rt_long %>%             # DONNÉES NON LISSÉES pour comparaison ******
#   dplyr::filter(propriete == 'transmittance',
#                 sample_id == 11670314)
...
x11670314_plot_sg
# ggsave('~/Desktop/paramètres_S-GOLAY_NONlissées(12-02-2020).pdf', plot = x11670314_plot_sg, height = 4, width = 4)
...
# Enregistrement de rt_sg
# save(rt_sg, file = "~/Documents/Maîtrise/DonnéesAnalyses/spectres/rt_sg.RData") # rt_sg_NO_doublon, fait 2 juin / refait 15 août ($site_class)/ refait 21 aout problèmes perdu !

# obtenir rt_sg_large
rt_sg_large <- dcast(rt_sg, sample_id + scientific_name + site + site_class + propriete ~ wavelength, 
                     value.var = "valeur_sg")
