...
# GROUPE TF "azote" (1-2): 300-2400 nm  / % N, C, C:N 

# section archives contient : QUICK PLS-R *
...
azote_df <- refl_ft %>% 
  dplyr::select(sample_id, N_pourc, C_N_ratio, "X400":"X2400")

# créer objets utilisés dans boucle 
inBands <- names(azote_df)[4:2003]
inVars <- names(azote_df)[2:3]
...
for (i in 1:length(inVars)){
  ...
  nCut <- floor(0.7 * nrow(azote_df)) # 80% data for calibration
  nComps <- 15                 
  nsims <-  100 # can be increased
  ...
  for (nsim in seq(nsims)){
    ...
    flush.console() # ?
    azote_df$RAND <- order(runif(nrow(azote_df))) # runif(n, min = , max = ) is used to generate n uniform (autant de fréquence de chaque tranche) random numbers lie in the interval (min, max).
    subData <- azote_df[azote_df["RAND"] < nCut, ] # choisi ligne < à ordre random, subset pour tester
  ...
  jpeg(paste("resultats/", "400-2400_", inVar, "/", inVar, "_abs_PRESS.jpg", sep = ""),
  ...
  jpeg(paste("resultats/", "400-2400_", inVar, "/", inVar,"_abs_RMSEP.jpg",sep = ""),
       width = 6, height = 5, units = "in", res = 200)
  par(bty = "l")
...
(inVar <- names(azote_df[2])) 

# subsetter saveData (issu de la partie 1)
azote_df_ITER <- saveData[complete.cases(saveData[,inVar]),]
...
for (i in 1:length(nCompss)){
  nsims <- 100
  nComps <- nCompss[i]
  nCut <- floor(0.7*nrow(azote_df_ITER))
  ...
  for (nsim in seq(nsims)){
    print(nsim)
    flush.console()
    azote_df_ITER$RAND <- order(runif(nrow(azote_df_ITER))) # same de partie 1 : juste créer chiffre random sur elsquels baser les subsets test et training
    subData <- azote_df_ITER[azote_df_ITER["RAND"] < nCut,]
    tstData <- azote_df_ITER[azote_df_ITER["RAND"] >= nCut,] # ">=" ne pas séparer
  ...
  write.csv(statMat, paste("resultats/", "400-2400_", inVar, "/", inVar, "_", nComps,"comps_abs_stats.csv", sep=""), row.names=FALSE)
  ...
  specMat <- azote_df_ITER[, inBands] # ne sort que les colonnes inBands d'azote_df_ITER
  ...
  preds <- azote_df_ITER[, !(names(azote_df_ITER) %in% inBands | names(azote_df_ITER) == "RAND")] # 2 critères de sélection des colonnes d'azote_df_ITER à mettre dans
  # preds : [ {base} "x | y" = "OR" ] / DONC pas les noms d'azote_df_ITER 
  # qui sont dans inBands OU METTRE les colonnes names(azote_df_ITER) == "RAND")...
  ...
  write.csv(preds, paste("resultats/", "400-2400_", inVar, "/", inVar, "_", nComps,"comps_abs_preds.csv", sep=""), row.names=FALSE)
  ...
  jpeg(paste("resultats/", "400-2400_", inVar,  "/", inVar, "_", nComps, "comps_abs_predplot.jpg", sep = ""),
       width = 6, height = 5, units = "in", res = 300)
  plot(formi, data = preds, pch = 16, cex = 0.8, ylab = "measured", xlab = "predicted",
       main = paste(inVar, nsims, nCut), xlim = c(min(predMean - predStdv), max(predMean + predStdv)))
  # abline(lm(lmformi, data = preds))
  abline(a = 0, b = 1, lty = 2)
  # # arrows(predMean, azote_df_ITER[, inVar], predMean + predStdv, azote_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  # arrows(predMean, azote_df_ITER[, inVar], predMean - predStdv, azote_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  ...
  # jpeg(paste("resultats/", "400-2400_", inVar,  "/", inVar, "_", nComps, "comps_abs_varimp.jpg", sep = ""),
  ...
  #      type = "p", pch = 19, xlab = "", ylab = "coeff_stdmean", main = paste(inVar, nComps, "comps", sep = "_", nsims, nCut),
...
# CHOIX DU TRAIT, un à la fois * 
(inVar <- names(azote_df[3])) 

# subsetter saveData (issu de la partie 1)
azote_df_ITER <- saveData[complete.cases(saveData[,inVar]),]

# choix de n comps selon partie 1
nCompss <- 8 # look at summary stats above and choose ncomps, muliple options are ok
...
for (i in 1:length(nCompss)){
  ...
  # par(mfrow = c(2,1), mar = c(1.5,4,2.5,1.5), oma = c(3,0,0,0))
  # plot(coeff_std$mean[-1] ~ as.numeric(substr(coeff_std$bands[-1], 2, nchar(coeff_std$bands[-1]))),
  #      type = "p", pch = 19, xlab = "", ylab = "coeff_stdmean", main = paste(inVar, nComps, "comps", sep = "_", "iter =", nsims, "nb", nCut),
  #      ylim = c(-max(abs(coeff_std$mean[-1])), max(abs(coeff_std$mean[-1]))), bty = "l")
  # abline(h = 0)
  # points(abs(coeff_std$mean)[-1] ~ as.numeric(substr(coeff_std$bands[-1], 2, nchar(coeff_std$bands[-1]))),
  #        xlab = "wvl", ylab = "coeff_stdmean", col = 2, pch = 16, cex = 0.8)
