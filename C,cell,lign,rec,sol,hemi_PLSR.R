...
# GROUPE TF "carbone" : "SWIR1-2" 1200:2400 nm / C %, cellulose, lignine, recalcitrant


# SECTION ARCHIVE CONTIENT : √ 
...
# créer le tableau de données et sélectionner les traits à rouler ensemble
carbone_df <- refl_ft %>% 
  dplyr::select(sample_id, C_pourc, soluble_contents,  "X1200":"X2400") #cellulose, lignine, recalcitrant, hemicellulose, ,)

# créer objets utilisés dans boucle 
inBands <- names(carbone_df)[4:1203]
inVars <- names(carbone_df)[2:3]
...
for (i in 1:length(inVars)){
  ...
  nCut <- floor(0.7 * nrow(carbone_df)) # 80% data for calibration
  nComps <- 15
  nsims <- 100
  ...
  for (nsim in seq(nsims)){
    ...
    flush.console() # ?
    carbone_df$RAND <- order(runif(nrow(carbone_df))) # runif(n, min = , max = ) is used to generate n uniform (autant de fréquence de chaque tranche) random numbers lie in the interval (min, max).
    subData <- carbone_df[carbone_df["RAND"] < nCut, ] # choisi ligne < à ordre random, subset pour tester
  ...
  jpeg(paste("resultats/", "1200-2400_", inVar, "/", inVar, "_abs_PRESS.jpg", sep = ""),
  ...
  jpeg(paste("resultats/", "1200-2400_", inVar, "/", inVar,"_abs_RMSEP.jpg",sep = ""),
       width = 6, height = 5, units = "in", res = 200)
  par(bty = "l")
...
(inVar <- names(carbone_df[2])) 

# subsetter saveData (issu de la partie 1)
carbone_df_ITER <- saveData[complete.cases(saveData[,inVar]),]
...
for (i in 1:length(nCompss)){
  nsims <- 100
  nComps <- nCompss[i]
  nCut <- floor(0.7*nrow(carbone_df_ITER))
  ...
  for (nsim in seq(nsims)){
    print(nsim)
    flush.console()
    carbone_df_ITER$RAND <- order(runif(nrow(carbone_df_ITER))) # same de partie 1 : juste créer chiffre random sur elsquels baser les subsets test et training
    subData <- carbone_df_ITER[carbone_df_ITER["RAND"] < nCut,]
    tstData <- carbone_df_ITER[carbone_df_ITER["RAND"] >= nCut,] # ">=" ne pas séparer
  ...
  write.csv(statMat, paste("resultats/", "1200-2400_", inVar, "/", inVar, "_", nComps,"comps_abs_stats.csv", sep=""), row.names=FALSE)
  ...
  specMat <- carbone_df_ITER[, inBands] # ne sort que les colonnes inBands d'carbone_df_ITER
  ...
  preds <- carbone_df_ITER[, !(names(carbone_df_ITER) %in% inBands | names(carbone_df_ITER) == "RAND")] # 2 critères de sélection des colonnes d'carbone_df_ITER à mettre dans
  # preds : [ {base} "x | y" = "OR" ] / DONC pas les noms d'carbone_df_ITER 
  # qui sont dans inBands OU METTRE les colonnes names(carbone_df_ITER) == "RAND")...
  ...
  write.csv(preds, paste("resultats/", "1200-2400_", inVar, "/", inVar, "_", nComps,"comps_abs_preds.csv", sep=""), row.names=FALSE)
  ...
  jpeg(paste("resultats/", "1200-2400_", inVar,  "/", inVar, "_", nComps, "comps_abs_predplot.jpg", sep = ""),
  ...
  # arrows(predMean, carbone_df_ITER[, inVar], predMean + predStdv, carbone_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  # arrows(predMean, carbone_df_ITER[, inVar], predMean - predStdv, carbone_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  ...
  # jpeg(paste("resultats/", "1200-2400_", inVar,  "/", inVar, "_", nComps, "comps_abs_varimp.jpg", sep = ""),
