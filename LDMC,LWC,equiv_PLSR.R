...
# GROUPE TF "structure" : "PLATEAU NIR" 700:1500 / jusqu'à 2000 => bandes d'absorption de l'eau 

# SECTION ARCHIVE CONTIENT : √ quick PLS-R
...
str_df <- refl_ft %>%
  dplyr::select(sample_id, leaf_dry_matter_content_mg_g, leaf_water_content_mg_g, equivalent_water_thickness_cm, "X800":"X2400")

# créer objets utilisés dans boucle 
inBands <- names(str_df)[5:1604]
inVars <- names(str_df)[2:4]
...
for (i in 1:length(inVars)){
  ...
  nCut <- floor(0.7 * nrow(str_df)) # 80% data for calibration
  nComps <- 15 #                    ///                            METTRE 5 POUR L'INSTANT
  nsims <- 100 # can be increased ///                             METTRE 3 POUR L'INSTANT
  ...
  for (nsim in seq(nsims)){
    ...
    flush.console() # ?
    str_df$RAND <- order(runif(nrow(str_df))) # runif(n, min = , max = ) is used to generate n uniform (autant de fréquence de chaque tranche) random numbers lie in the interval (min, max).
    subData <- str_df[str_df["RAND"] < nCut, ] # choisi ligne < à ordre random, subset pour tester
...
(inVar <- names(str_df[2])) 

# subsetter saveData (issu de la partie 1)
str_df_ITER <- saveData[complete.cases(saveData[,inVar]),]

# choix de n comps selon partie 1
nCompss <- 5 # look at summary stats above and choose ncomps, muliple options are ok
...
for (i in 1:length(nCompss)){
  nsims <- 100
  nComps <- nCompss[i]
  nCut <- floor(0.7*nrow(str_df_ITER))
  ...
  for (nsim in seq(nsims)){
    print(nsim)
    flush.console()
    str_df_ITER$RAND <- order(runif(nrow(str_df_ITER))) # same de partie 1 : juste créer chiffre random sur elsquels baser les subsets test et training
    subData <- str_df_ITER[str_df_ITER["RAND"] < nCut,]
    tstData <- str_df_ITER[str_df_ITER["RAND"] >= nCut,] # ">=" ne pas séparer
...
#              sep = ",", col.names = T, row.names = F)
  ...
  specMat <- str_df_ITER[, inBands] # ne sort que les colonnes inBands d'str_df_ITER
  ...
  preds <- str_df_ITER[, !(names(str_df_ITER) %in% inBands | names(str_df_ITER) == "RAND")] # 2 critères de sélection des colonnes d'str_df_ITER à mettre dans
  # preds : [ {base} "x | y" = "OR" ] / DONC pas les noms d'str_df_ITER 
  # qui sont dans inBands OU METTRE les colonnes names(str_df_ITER) == "RAND")...
...
#  abline(lm(lmformi, data = preds))
  ...
  # # arrows(predMean, str_df_ITER[, inVar], predMean + predStdv, str_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
  # arrows(predMean, str_df_ITER[, inVar], predMean - predStdv, str_df_ITER[, inVar], angle = 90, length = 0.05, lwd = 0.8)
...
# CHOIX DU TRAIT, un à la fois * 
(inVar <- names(str_df[4])) 

# subsetter saveData (issu de la partie 1)
str_df_ITER <- saveData[complete.cases(saveData[,inVar]),]

# choix de n comps selon partie 1
nCompss <- 6 # look at summary stats above and choose ncomps, muliple options are ok
